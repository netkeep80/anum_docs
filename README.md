...
> **Кандидат следующего выпуска.** Пара `mts-contract/v0.10` + `mts-conformance/v0.10` фиксирует исследуемую нотацию `:` / `.` как отдельную непринятую границу. Она имеет `accepted=false` и `acceptanceReady=false`; текущим выпуском остаётся МТС v0.9. Для основания знака, контекстного связывания точки и отрицательных границ уже существуют исполняемые тесты; это ещё не означает принятия v0.10.
