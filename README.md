...
> **Кандидат следующего выпуска.** Пара `mts-contract/v0.10` + `mts-conformance/v0.10` фиксирует исследуемую нотацию `:` / `.` как отдельную непринятую границу. Она имеет `accepted=false` и `acceptanceReady=false`; текущим выпуском остаётся МТС v0.9.
