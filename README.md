...
> **Кандидат следующего выпуска.** Пара `mts-contract/v0.11` + `mts-conformance/v0.11` фиксирует непринятую границу верхнеуровневого корневого связывания точки: `TopBind(R,S)` и `resolve_top(.) = R`. У кандидата `accepted=false`, `acceptanceReady=false`; этапы `C1` / `C2` и производственная реализация не завершены. Текущим выпуском остаётся МТС v0.10.
