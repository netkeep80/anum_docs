...
> **Кандидат следующего выпуска.** Пара `mts-contract/v0.10` + `mts-conformance/v0.10` фиксирует исследуемую нотацию `:` / `.` как отдельную непринятую границу. Она имеет `accepted=false` и `acceptanceReady=false`; текущим выпуском остаётся МТС v0.9. Для основания знака, контекстного связывания точки, однополюсных рекурсивных fixed-point законов и отрицательных границ уже существуют исполняемые тесты; это ещё не означает принятия v0.10.
...
`♂e` и `b♀` здесь — остенсивная теоретическая запись, а не команды получения полюсов. В принятом F1 МТС v0.9 знаки `♂` и `♀` не входят во внутренний минимальный реестр и остаются отложенными.
...
Принятое основание МТС v0.9 сохраняет пятисвязное ядро:
...
Минимальный внутренний алфавит F1 МТС v0.9 содержит ровно:
...
В МТС v0.9 контекст не определяется скрытым стеком среды исполнения:
...
`I` — явно выбранный интерпретатор, `K` — связевая форма контекста. Лексическая вложенность хранится через `K.parent`.

Открытие и закрытие контекстов `Q` и `FORMAL`, а также возврат результата родителю оформляются явными структурными переходами. Временной порядок хранится в `Run`/`Act`.

`current(K)` здесь — только метанотация чтения структуры `K`; внутренний знак `↑` в F1 v0.9 не принят.
...
МТС v0.9 принимает каноническое представление `STRING→QUATERNARY`: каждый физический байт кодируется одним конвертом `[8 bits]`. Нормализация `Unicode` не выполняется скрыто, а технический идентификатор байта не становится смысловым тождеством.
...
Формальное `=` в МТС v0.9 использует явное контекстно-локальное свидетельство представителя. В метанотации:
...
## 13. Производные результаты F2–F6

Завершённый цикл вывел поверх основания, не добавляя новые корневые виды сущностей:

- конечную нормализацию и теорему об отсутствии смысловых экземпляров без структурного различия;
- рекурсивную реверсию `J`, отдельную от перестановки полюсов и обратного отношения навигации;
- счёт по степеням связности и производную структуру Пеано;
- конечные коллекции через явные вхождения и происхождение;
- конечные функции как декларативные данные;
- относительную навигацию и алгебру путей;
- прикладные интерпретации только под явной теорией и контекстом.
...
3. [Формальная нотация МТС](docs/specs/Формальная%20нотация%20МТС.md) — знаки F1, контексты `Q`/`FORMAL` и правила чтения.
...
МТС v0.9 имеет `semanticBase = mts-contract/v0.8` и `observableSemanticDelta = true`.
