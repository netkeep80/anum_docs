...
> **Кандидат следующего выпуска.** Пара `mts-contract/v0.11` + `mts-conformance/v0.11` фиксирует непринятую границу верхнеуровневого корневого связывания точки: `TopBind(R,S)` и `resolve_top(.) = R`. У кандидата `accepted=false`, `acceptanceReady=true`: этапы `C1`–`C5`, производственная реализация и требуемое исполняемое покрытие завершены, поэтому кандидат готов к отдельному решению о принятии. Это состояние ещё не принимает v0.11: отдельные этапы `C6-B0/B1` должны выполнить явное принятие и переключение выпуска, а `C6-B2` — погасить временный бюджет контрактов `6/2`. Текущим принятым выпуском остаётся МТС v0.10.
