...
Корень не подставляется скрытым исходным префиксом и не берётся из ambient runtime current: верхнеуровневая точка разрешается через явное структурное связывание.
