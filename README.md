...
> **Кандидат следующего выпуска готов к отдельному принятию.** Пара `mts-contract/v0.10` + `mts-conformance/v0.10` имеет `accepted=false` и `acceptanceReady=true`; полнота требуемых исполняемых свидетельств зафиксирована машинно. До отдельного перехода текущим и принятым выпуском остаётся МТС v0.9.
