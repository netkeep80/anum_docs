...
> **Кандидат следующего выпуска.** Пара `mts-contract/v0.11` + `mts-conformance/v0.11` фиксирует непринятую границу верхнеуровневого корневого связывания точки: `TopBind(R,S)` и `resolve_top(.) = R`. У кандидата `accepted=false`, `acceptanceReady=false`. Этап `C1` подтверждён исполняемым свидетельством `ts/test/research-physical-dot-bootstrap.test.ts` из PR `#763`. Этап `C2` классифицирован исполняемым свидетельством `ts/test/research-minimal-sign-basis-c2.test.ts` из PR `#770`: семантические значения `:` / `.` выводятся без независимых примитивов, но знаки представления сохраняются, а текущая самоопределяющая запись через `:` сохраняет цикл представления. Этап `C3` реализован и подтверждён производственным свидетельством `ts/test/v011-top-level-root-binding.test.ts` из PR `#782`: верхнеуровневая точка разрешается в `R` через явный `K₀`, а две точки сохраняют две позиции до `Pair(R,R)=R`. Этап `C4` реализован и подтверждён производственным свидетельством `ts/test/v011-nested-explicit-binding-compatibility.test.ts` из PR `#788`: вложенный `A:E` сохраняет ближайшее структурное связывание без скрытого текущего контекста и без семантического PUSH/POP. Производственная реализация смыслового изменения v0.11 теперь полна; совместимость границы `Q` и принятие кандидата ещё не завершены. Текущим выпуском остаётся МТС v0.10.
