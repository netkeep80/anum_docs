...
> **Кандидат следующего выпуска.** Пара `mts-contract/v0.11` + `mts-conformance/v0.11` фиксирует непринятую границу верхнеуровневого корневого связывания точки: `TopBind(R,S)` и `resolve_top(.) = R`. У кандидата `accepted=false`, `acceptanceReady=false`. Этапы `C1`–`C4` подтверждены ранее связанными исполняемыми и производственными свидетельствами. Этап `C5` подтверждён свидетельством `ts/test/v011-q-boundary-compatibility.test.ts` из PR `#792`: алфавит `Q` остаётся ровно `[ ] 1 0`, `.` и `:` отвергаются как не-абиты, контекстное связывание не наследуется внутрь `Q`, а изменения производственной среды для этого не потребовались. Производственная реализация и требуемое исполняемое покрытие кандидата v0.11 теперь полны; отдельными обязательствами `C6` остаются явное принятие/переключение выпуска и погашение временного бюджета контрактов `6/2`. Текущим принятым выпуском остаётся МТС v0.10.
