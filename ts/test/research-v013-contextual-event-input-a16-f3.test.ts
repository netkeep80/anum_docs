...
function assert(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(`v0.13 A16-F3 contextual event input: ${message}`);
}
...
interface Artifact {
  readonly schema: "mts-v013-contextual-event-input/research-v0.1";
...
function buildArtifact(noise: boolean): Artifact {
  ...
  const coord = (link: LinkHandle): number => {
    ...
    assert(n !== undefined, "A16-F3 coordinate");
    return n;
  };

  return Object.freeze({
    schema: "mts-v013-contextual-event-input/research-v0.1" as const,
...
function replay(artifact: Artifact): Replay {
  same(artifact.schema, "mts-v013-contextual-event-input/research-v0.1", "A16-F3 schema");
  const memory = restoreTopology(artifact.topology);
  const all = memory.allLinks();
  const at = (n: number): LinkHandle => {
    const x = all[n];
    assert(x !== undefined, "A16-F3 replay coordinate");
    return x;
  };
  return Object.freeze({
    memory,
...
function propagateCallTruth(
  ...
  for (const parent of callFrontier.occurrences) {
    ...
    assert(contextual.start === context, "A16-F3 contextual call starts at K");
...
/**
 * Generic pair actualization over contextual truth operands.
 *
 * This kernel has no State/Event semantics. Given selected left/right truth
 * occurrences K->X and K->Y, it constructs X->Y and contextualizes that pair
 * as K->(X->Y). Which two truth frontiers are supplied remains an explicit
 * residual of this gate; arbitrary values merely coexisting under K are NOT
 * automatically cross-related.
 */
function actualizePairTruth(
  memory: Memory,
  context: LinkHandle,
  ...
  for (const l of left) {
    ...
    assert(lp.start === context, "A16-F3 left operand truth starts at K");

    for (const r of rightTruthFrontier.occurrences) {
      const rp = memory.poles(r.link);
      assert(rp.start === context, "A16-F3 right operand truth starts at K");

      const pair = memory.ensure(lp.end, rp.end);
...
function stateEnds(
  ...
  for (const truthLink of truthFrontier.links) {
    ...
    same(p.start, r.named.K!, "A16-F3 derived truth starts at K");
    states.add(p.end);
  }
  return states;
}
...
function normalizedProvenance(r: Replay, bundle: BundleValue): readonly string[] {
  ...
  return Object.freeze(bundle.occurrences.map((o) => {
    ...
    assert(n !== undefined, "A16-F3 consequence coordinate");
...
function assertBranchLocalStates(r: Replay): void {
  ...
  "A16-F3 first split keeps branch-local meta-state");
  const ka = readContext(r.memory, A.context);
  const kb = readContext(r.memory, B.context);
  same(ka.parent, kb.parent, "A16-F3 first split preserves lexical parent");
  ...
  assert(C.context !== D.context && C.tos !== D.tos && C.currentFn !== D.currentFn,
    "A16-F3 recursive split remains branch-local");
}
...
function exercise(artifact: Artifact): void {
  ...
  for (const name of ["K", "E0", "EA", "EB", "EC", "ED", "EZ", "event1", "event2", "event3"]) {
    assert(n[name] !== undefined, `A16-F3 named ${name}`);
  }
  assertBranchLocalStates(r);

  const start = initialTruth(r, n.E0!);
  setSame(stateEnds(r, start), [n.E0!], "A16-F3 initial K->E0 truth");

  const event1Truth = contextualTruth(r, n.event1!);
  const firstCalls = actualizePairTruth(
    r.memory, n.K!, start, event1Truth, "forward",
  );
  const first = propagateCallTruth(r.memory, n.K!, firstCalls, r.transitions);
  setSame(stateEnds(r, first), [n.EA!, n.EB!], "A16-F3 first split");
  same(first.occurrences.length, 2, "A16-F3 first branch count");

  const event2Truth = contextualTruth(r, n.event2!);
  const secondCallsForward = actualizePairTruth(
    r.memory, n.K!, first, event2Truth, "forward",
  );
  const secondCallsReverse = actualizePairTruth(
  ...
  setSame(stateEnds(r, secondForward), [n.EC!, n.ED!], "A16-F3 recursive split");
  same(secondForward.occurrences.length, 2,
    "A16-F3 EA terminates while EB splits");
  assert(valuesEqual(secondForward, secondReverse),
    "A16-F3 schedule order preserves extensional truth frontier");
  exactJson(
    normalizedProvenance(r, secondForward),
    normalizedProvenance(r, secondReverse),
    "A16-F3 schedule order preserves provenance",
  );
  exactJson(
    secondForward.occurrences.map((o) => o.path).sort(),
    [[1, 0], [1, 1]],
    "A16-F3 terminated EA branch contributes no truth children",
  );

  const event3Truth = contextualTruth(r, n.event3!);
  const thirdCallsForward = actualizePairTruth(
    r.memory, n.K!, secondForward, event3Truth, "forward",
  );
  const thirdCallsReverse = actualizePairTruth(
    r.memory, n.K!, secondReverse, event3Truth, "reverse",
  );
  ...
  setSame(stateEnds(r, thirdForward), [n.EZ!], "A16-F3 converged contextual truth");
  same(thirdForward.links.size, 1, "A16-F3 one extensional K->EZ truth Link");
  same(thirdForward.occurrences.length, 2,
    "A16-F3 convergence preserves two proof occurrences");
  assert(valuesEqual(thirdForward, thirdReverse),
    "A16-F3 convergence ignores scheduler order");
  exactJson(
    normalizedProvenance(r, thirdForward),
    normalizedProvenance(r, thirdReverse),
    "A16-F3 converged provenance ignores scheduler order",
  );

  // Generic application construction is not semantic authority.
  // EA/event3 was not one of the frozen selected applications. A16-F3 creates
  ...
  // resulting call truth must evaluate to ZERO.
  {
    ...
    const unknownCalls = actualizePairTruth(
      r.memory, n.K!, onlyEA, event3Truth, "forward",
    );
    const unknown = propagateCallTruth(r.memory, n.K!, unknownCalls, r.transitions);
    same(unknown.links.size, 0,
      "A16-F3 generic application without selected continuation yields ZERO");
    same(unknown.occurrences.length, 0,
      "A16-F3 unknown application contributes no provenance");
  }
  ...
  const firstAgain = propagateCallTruth(
    r.memory,
    n.K!,
    actualizePairTruth(r.memory, n.K!, start, event1Truth, "forward"),
    r.transitions,
  );
  setSame(stateEnds(r, firstAgain), [n.EA!, n.EB!],
    "A16-F3 ambient event truth outside selected operand frontier ignored");
  ...
  const mixedCalls = actualizePairTruth(
  ...
  setSame(stateEnds(r, mixedResult), [n.EA!, n.EB!],
    "A16-F3 unknown contextual event branch yields no semantic result");
  ...
  setSame(stateEnds(r, afterAmbient), [n.EC!, n.ED!],
    "A16-F3 late ambient transition ignored");
  ...
  setSame(stateEnds(r, afterAmbientTruth), [n.EC!, n.ED!],
    "A16-F3 ambient K->state outside frontier ignored");
}

function staticKernelGuard(): void {
  const repoRoot = resolve(process.cwd(), "..");
  const source = readFileSync(
    join(repoRoot, "ts/test/research-v013-contextual-event-input-a16-f3.test.ts"),
    "utf8",
  );
  ...
  for (const forbidden of [
    ...
    assert(!transitionKernel.includes(forbidden),
      `A16-F3 propagation kernel excludes host semantic primitive ${forbidden}`);
  }
  ...
  for (const forbidden of [
    ...
    assert(!pairKernel.includes(forbidden),
      `A16-F3 pair actualization is value-generic and excludes ${forbidden}`);
  }

  assert(pairKernel.includes("leftTruthFrontier: BundleValue"),
    "A16-F3 left operand is contextual truth frontier");
  assert(pairKernel.includes("rightTruthFrontier: BundleValue"),
    "A16-F3 right operand is contextual truth frontier");
}

function main(): void {
  const a = buildArtifact(false);
  const b = buildArtifact(true);
  exactJson(a, b, "A16-F3 portable authority ignores unrelated allocation noise");
