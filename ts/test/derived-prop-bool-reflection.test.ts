...
function fixture() {
  ...
  return {
    memory, basis, theory, foreignTheory, fresh,
    distinctOC, distinctCO, distinctOCPack, distinctCOPack,
    forwardL, forwardU, reverseL, reverseU,
    eqProp, neqProp, eqValue,
    resolvedNode, distinctRoot, eqPropNode, neqPropNode, directLNode, directUNode,
...
// Missing proof occurrence is not a negative proof and cannot synthesize U.
{
  const f = fixture(), A = f.fresh(), B = f.fresh();
  const K = defineContext(f.memory, f.basis.R, f.basis.O);
  const r = f.resolvedNode(K, A, B, f.basis.O, f.basis.O);
...
// Wrong reflection direction, alternate value atom, K mismatch and foreign admission all fail structurally.
{
  const f = fixture(), A = f.fresh(), B = f.fresh(), alt = f.fresh();
  const K = defineContext(f.memory, f.basis.R, f.basis.O);
  ...
  const altPackRoles = [f.fresh(), f.fresh(), f.fresh()] as const;
  const [ak, aa, ab] = altPackRoles;
  const forgedClaim = f.eqValue(K, A, B, alt);
  // Reuse reverse-L rule with a claim whose value slot is not L: structural premise matching must reject it.
  const forgedOccurrence = defineStructuralProofOccurrence(f.memory, valueL.node.judgment.application.act, forgedClaim);
  const altReverse = f.reverseLNode(K, A, B, forgedOccurrence);
  reject(() => f.replay([r.equality], f.proof(altReverse, [r, valueL, altReverse])), "alternate value atom has no reflection authority");
  void ak; void aa; void ab;
