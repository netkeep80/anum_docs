...
function frontierTruthLinks(memory:Memory,E:LinkHandle):readonly LinkHandle[]{const ep=memory.poles(E),out:LinkHandle[]=[];for(const occurrence of readChain(memory,ep.end,"frontier")){const truthLink=memory.poles(occurrence).end,truth=memory.poles(truthLink);same(truth.start,ep.start,"A63 frontier truth-link context");out.push(truthLink);}return Object.freeze(out);}
...
function proveOutputs(memory:Memory,K:LinkHandle,rule:LinkHandle,outputs:readonly LinkHandle[],values:ReadonlyMap<LinkHandle,LinkHandle>):LinkHandle|undefined{if(outputs.length===0)return undefined;let stage=memory.ensure(rule,memory.root);const checks:EqualityCheck[]=[];for(const output of outputs){const actual=values.get(output);if(actual===undefined)return undefined;stage=memory.ensure(stage,memory.ensure(output,actual));checks.push(equality(memory,stage,output,actual));}const transitions=checks.map((x,i)=>memory.ensure(x.gate,checks[i+1]?.query??rule)),context=memory.ensure(K,freezeChain(memory,transitions)),seed=memory.ensure(context,checks[0]!.query);let E=memory.ensure(context,freezeChain(memory,[memory.ensure(memory.root,seed)]));for(let i=0;i<checks.length;i+=1)E=step(memory,E,"forward");const truths=frontierTruthLinks(memory,E);return truths.length===1?truths[0]:undefined;}
function selfRealize(memory:Memory,f:Frame,rule:LinkHandle,admissionContext:LinkHandle,schedule:"forward"|"reverse",constructor:PairConstructor):LinkHandle|undefined{try{const structural=readStructuralRule(memory,rule),dictionary=readStructuralRoleDictionary(memory,structural.roleDictionary),body=memory.poles(structural.body),obligations=[...readExactSequence(memory,body.start).values],outputs=readExactSequence(memory,body.end).values,values=new Map<LinkHandle,LinkHandle>(dictionary.roles.map(role=>[role,role] as const)),derived=new Set(obligations.map(w=>readObligation(memory,w).node));let pending=obligations;while(pending.length>0){let progress=false;const scan=schedule==="forward"?pending:[...pending].reverse(),done=new Set<LinkHandle>();for(const w of scan){const o=readObligation(memory,w),left=values.get(o.left)??(dictionary.roles.includes(o.left)||derived.has(o.left)?undefined:o.left),right=values.get(o.right)??(dictionary.roles.includes(o.right)||derived.has(o.right)?undefined:o.right);if(left===undefined||right===undefined)continue;const result=executeApplication(memory,f,selectedApplication(memory,rule,f,w,o.left,o.right,left,right,o.node),constructor);if(result===undefined)return undefined;if(!values.has(o.node))values.set(o.node,result);done.add(w);progress=true;}if(!progress)return undefined;pending=pending.filter(x=>!done.has(x));}return proveOutputs(memory,admissionContext,rule,outputs,values);}catch{return undefined;}}
...
function exercise(memory:Memory,noise:boolean,factory:ConstructorFactory):void{const b=ensureRootBasis(memory);if(noise)memory.ensure(memory.ensure(b.U,b.C),b.O);const f=frame(memory,b),forward=defineSchema(memory,f,b,false),reverse=defineSchema(memory,f,b,true),K=memory.ensure(b.R,b.L);for(const s of [forward,reverse])for(const schedule of ["forward","reverse"] as const){const admission=selfRealize(memory,f,s.rule,K,schedule,factory(memory));assert(admission!==undefined,"A63 valid Rule self-realizes");const p=memory.poles(admission),kp=memory.poles(p.start);same(kp.start,K,"A63 admission execution context descends from selected context");same(p.end,s.rule,"A63 admission carries exact Rule");}
