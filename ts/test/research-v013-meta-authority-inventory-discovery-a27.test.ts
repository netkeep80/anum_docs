...
function assert(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(`v0.13 A27 meta inventory discovery: ${message}`);
}
...
function readChain(memory: Memory, envelope: LinkHandle): readonly LinkHandle[] {
  ...
  assert(e.start === envelope && e.end !== envelope, "A27 proper START-self envelope");
  const out: LinkHandle[] = [];
  const seen = new Set<LinkHandle>();
  let cursor = e.end;
  while (cursor !== memory.root) {
    assert(!seen.has(cursor), "A27 chain cycle");
    seen.add(cursor);
...
function referenceNextExecution(memory: Memory, executionRoot: LinkHandle): LinkHandle {
  ...
  for (const occurrence of occurrences) {
    ...
    assert(truth.start === K, "A27 reference truth keeps K");
...
function buildFamily(memory: Memory, b: RootBasis, seed: LinkHandle): Family {
  ...
  const at=(i:number):LinkHandle=>{const v=fresh[i];assert(v!==undefined,`A27 fresh ${i}`);return v;};
...
/** Exact A23 runtime consumer; A27 changes only producer-authority source selection. */
function metaStep(
  memory: Memory,
...
interface Template { readonly candidate:LinkHandle; readonly M:LinkHandle; readonly publication:LinkHandle; }
function directTemplate(memory:Memory,rule:LinkHandle,f:Family):Template{
  ...
  const env=memory.ensureStartSelfClosed(cells[0]!), meta=memory.ensure(f.metaParent,env);
  const candidate=materializeExactSequence(memory,[
    memory.root,f.metaParent,...es,...ts,cells[3]!,cells[2]!,cells[1]!,cells[0]!,env,meta,
  ]);
  assert(admitted(memory,rule,candidate),"A27 source template admitted");
...
function seedMap(memory:Memory,correspondence:LinkHandle):Map<LinkHandle,LinkHandle>{
  const p=memory.poles(correspondence);
  const a=readExactSequence(memory,p.start).values,b=readExactSequence(memory,p.end).values;
  assert(a.length===b.length,"A27 correspondence cardinality");
  const map=new Map<LinkHandle,LinkHandle>([[memory.root,memory.root]]);
  for(let i=0;i<a.length;i+=1){
    const s=a[i],t=b[i];assert(s!==undefined&&t!==undefined,"A27 complete correspondence");
    const prior=map.get(s);assert(prior===undefined||prior===t,"A27 correspondence conflict");map.set(s,t);
  }
  return map;
}
function instantiate(memory:Memory,source:LinkHandle,map:Map<LinkHandle,LinkHandle>):LinkHandle{
  ...
  const prior=map.get(source);assert(prior===undefined||prior===target,"A27 clone conflict");map.set(source,target);
  return target;
}

function packageTemplate(memory:Memory,t:Template,sourceSeeds:LinkHandle,targetSeeds:LinkHandle):LinkHandle{
  return materializeExactSequence(memory,[t.candidate,memory.ensure(sourceSeeds,targetSeeds)]);
}
function runInventory(
  ...
  const packages=readExactSequence(memory,inventory).values;
  assert(packages.length>0,"A27 non-empty broad template inventory");
  const candidates=new Set<LinkHandle>(), publications=new Set<LinkHandle>(), metas=new Set<LinkHandle>();
  let compatibleCount=0;

  for(const pkg of packages){
    const q=readExactSequence(memory,pkg).values;
    same(q.length,2,"A27 package arity");
    const source=q[0],correspondence=q[1];
    assert(source!==undefined&&correspondence!==undefined,"A27 complete package");

    // Compatibility is not prefiltered by the host. It is derived from the
    // Link-carried correspondence target: only packages whose correspondence
    // ends at the selected target seed sequence belong to this target frontier.
    const cp=memory.poles(correspondence);
    if(cp.end!==targetSeeds) continue;
    compatibleCount+=1;

    const candidate=instantiate(memory,source,seedMap(memory,correspondence));
    assert(admitted(memory,rule,candidate),"A27 discovered target candidate admitted");
    const values=readExactSequence(memory,candidate).values;
    const meta=values[M];assert(meta!==undefined,"A27 target M");
    const publication=memory.ensure(candidate,meta);
    candidates.add(candidate);publications.add(publication);metas.add(meta);
  }

  assert(compatibleCount>0,"A27 at least one structurally compatible template");
  same(candidates.size,1,"A27 discovered compatible templates converge candidate");
  same(publications.size,1,"A27 discovered compatible templates converge publication");
  same(metas.size,1,"A27 discovered compatible templates converge M");
...
function exercise(noise:boolean):void{
  ...
  const t1=directTemplate(memory,rule,source1);
  const t2=directTemplate(memory,rule,source2);
  const t3=directTemplate(memory,rule,source3);
  assert(
    new Set([t1.candidate,t2.candidate,t3.candidate]).size===3,
    "A27 broad inventory source template identities distinct",
  );

  const targetSeeds=seedSequence(memory,target);
  const s1=seedSequence(memory,source1);
  const s2=seedSequence(memory,source2);
  const s3=seedSequence(memory,source3);

  const p1=packageTemplate(memory,t1,s1,targetSeeds);
  const p2=packageTemplate(memory,t2,s2,targetSeeds);

  // p3 is a valid package but incompatible with the selected target because
  // its correspondence ends at a different seed sequence.
  const p3=packageTemplate(memory,t3,s3,s3);

  same(memory.find(target.E0,target.E1),undefined,
    "A27 target authority absent at freeze");

  // The frozen inventory is intentionally broader than the compatible subset.
  const inventory=materializeExactSequence(memory,[p1,p2,p3]);
  const out=runInventory(memory,rule,inventory,targetSeeds);
  same(out.inventoryCount,3,"A27 broad inventory size");
  same(out.compatibleCount,2,"A27 structurally discovered compatible subset size");

  let truth=memory.ensure(out.M,target.E0);
  for(const expected of [target.E1,target.E2,target.E3,target.E4]){
    const next=metaStep(memory,truth);assert(next!==undefined,"A27 target meta-step");
    same(memory.poles(next).end,expected,"A27 discovered authority execution");truth=next;
  }
  same(metaStep(memory,truth),undefined,"A27 terminal ZERO");

  // A late ambient compatible package is inert because it was not a member
  // of the frozen broad inventory.
  packageTemplate(memory,t3,s3,targetSeeds);
  const afterAmbient=runInventory(memory,rule,inventory,targetSeeds);
  same(afterAmbient.inventoryCount,3,"A27 late ambient package does not mutate inventory");
  same(afterAmbient.compatibleCount,2,"A27 late ambient compatible package ignored");
  same(afterAmbient.candidate,out.candidate,"A27 ambient package preserves target candidate");

  // A package is first structurally selected by correspondence target. Once
  // selected, malformed correspondence evidence must fail closed, not be
  // silently skipped as incompatible.
  const badSource=materializeExactSequence(memory,[source1.metaParent,source1.E0]);
  const badSelected=materializeExactSequence(memory,[
    t1.candidate,memory.ensure(badSource,targetSeeds),
  ]);
  expectThrows(
    ()=>{runInventory(
      memory,rule,materializeExactSequence(memory,[p1,p2,p3,badSelected]),targetSeeds,
    );},
    "A27 malformed target-compatible evidence rejects discovered frontier",
  );
}
...
function staticGuards():void{
  ...
  const own=readFileSync(
    join(root,"ts/test/research-v013-meta-authority-inventory-discovery-a27.test.ts"),
    "utf8",
  );
  const prior=readFileSync(
    join(root,"ts/test/research-v013-meta-transition-authority-a23.test.ts"),
    "utf8",
  );
  const a=own.slice(
  ...
  same(a.replace(/\s+/g,""),b.replace(/\s+/g,""),"A27 unchanged A23 metaStep");
  ...
  for(const x of [
    "source1","source2","source3","t1","t2","t3","switch(","allLinks(",".outgoing(",".incoming(",
  ]) {
    assert(!runner.includes(x),`A27 inventory runner excludes template/ambient selector ${x}`);
  }
  assert(runner.includes("if(cp.end!==targetSeeds) continue"),
    "A27 compatibility is derived from Link-carried correspondence target");
}

function main():void{
  exercise(false);exercise(true);staticGuards();
  console.log([
    "MTS v0.13 A27: META_AUTHORITY_INVENTORY_DISCOVERY=GREEN_SCOPED_RESEARCH",
    "BROAD_INVENTORY_SIZE=3 DISCOVERED_COMPATIBLE_TEMPLATES=2 INCOMPATIBLE_VALID_TEMPLATE=EXCLUDED",
    "EXTERNAL_PREFILTERED_COMPATIBLE_FRONTIER=REMOVED HOST_SELECTED_TEMPLATE_HANDLE=NO",
    "DISCOVERY_LAW=CORRESPONDENCE_END_EQUALS_SELECTED_TARGET_SEEDS",
    ...
    "UNCHANGED_A23_META_STEP=YES LATE_AMBIENT_COMPATIBLE_PACKAGE=IGNORED",
    "MALFORMED_TARGET_COMPATIBLE_EVIDENCE=REJECTED",
    "EXTERNAL_BROAD_INVENTORY_MEMBERSHIP=YES EXTERNAL_SEQUENCE_CORRESPONDENCE=YES",
