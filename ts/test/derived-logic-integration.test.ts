...
// OR branch evidence is structural; a host label cannot turn Q into the LEFT premise P.
{
  ...
  const labelled: Built = Object.freeze({
...
// Raw implication authority is Theory-local; missing structural premises remain missing despite host order/metadata.
{
  ...
  const foreignTarget = (() => {
    const act = defineActHeader(f.memory, f.foreign.interpreter, f.mp.roleDictionary, K);
    const roles = Array.from(f.memory.outgoing(f.mp.roleDictionary));
    assert(roles.length >= 2, "MP role dictionary exposes role occurrences");
    return f.mpNode(P, Q, [p.occurrence, implication.occurrence], K);
  })();
  const forgedForeign: Built = Object.freeze({
  ...
  void act;
  reject(
    () => replayStructuralDerivation(f.memory, f.derivation(forgedForeign, [forgedForeign, p, implication], f.foreignTheory)),
