...
function assert(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(`v0.13 A26 meta template convergence: ${message}`);
}
...
function readChain(memory: Memory, envelope: LinkHandle): readonly LinkHandle[] {
  ...
  assert(e.start === envelope && e.end !== envelope, "A26 proper START-self envelope");
  const out: LinkHandle[] = [];
  const seen = new Set<LinkHandle>();
  let cursor = e.end;
  while (cursor !== memory.root) {
    assert(!seen.has(cursor), "A26 chain cycle");
    seen.add(cursor);
...
function referenceNextExecution(memory: Memory, executionRoot: LinkHandle): LinkHandle {
  ...
  for (const occurrence of occurrences) {
    ...
    assert(truth.start === K, "A26 reference truth keeps K");
...
function buildFamily(memory: Memory, b: RootBasis, seed: LinkHandle): Family {
  ...
  const at=(i:number):LinkHandle=>{const v=fresh[i];assert(v!==undefined,`A26 fresh ${i}`);return v;};
...
/** Exact A23 runtime consumer; A26 changes only producer-authority source selection. */
function metaStep(
  memory: Memory,
...
function directTemplate(memory:Memory,rule:LinkHandle,f:Family):Template{
  ...
  assert(admitted(memory,rule,candidate),"A26 source template admitted");
...
function seedMap(memory:Memory,correspondence:LinkHandle):Map<LinkHandle,LinkHandle>{
  ...
  assert(a.length===b.length,"A26 correspondence cardinality");
  const map=new Map<LinkHandle,LinkHandle>([[memory.root,memory.root]]);
  for(let i=0;i<a.length;i+=1){
    const s=a[i],t=b[i];assert(s!==undefined&&t!==undefined,"A26 complete correspondence");
    const prior=map.get(s);assert(prior===undefined||prior===t,"A26 correspondence conflict");map.set(s,t);
  }
  return map;
}
function instantiate(memory:Memory,source:LinkHandle,map:Map<LinkHandle,LinkHandle>):LinkHandle{
  ...
  const prior=map.get(source);assert(prior===undefined||prior===target,"A26 clone conflict");map.set(source,target);
  return target;
}
...
function runFrontier(memory:Memory,rule:LinkHandle,frontier:LinkHandle):Readonly<{candidate:LinkHandle;publication:LinkHandle;M:LinkHandle}>{
  const packages=readExactSequence(memory,frontier).values;
  assert(packages.length>0,"A26 non-empty template frontier");
  ...
  for(const pkg of packages){
    ...
    same(q.length,2,"A26 package arity"); const source=q[0],correspondence=q[1];
    assert(source!==undefined&&correspondence!==undefined,"A26 complete package");
    const candidate=instantiate(memory,source,seedMap(memory,correspondence));
    assert(admitted(memory,rule,candidate),"A26 target candidate admitted");
    const values=readExactSequence(memory,candidate).values;
    const meta=values[M];assert(meta!==undefined,"A26 target M");
    const publication=memory.ensure(candidate,meta);
    candidates.add(candidate);publications.add(publication);metas.add(meta);
  }
  same(candidates.size,1,"A26 compatible templates converge candidate");
  same(publications.size,1,"A26 compatible templates converge publication");
  same(metas.size,1,"A26 compatible templates converge M");
...
function exercise(noise:boolean):void{
  ...
  const t1=directTemplate(memory,rule,source1),t2=directTemplate(memory,rule,source2);
  assert(t1.candidate!==t2.candidate,"A26 independent source template identities");

  const targetSeeds=seedSequence(memory,target);
  const p1=packageTemplate(memory,t1,seedSequence(memory,source1),targetSeeds);
  const p2=packageTemplate(memory,t2,seedSequence(memory,source2),targetSeeds);
  same(memory.find(target.E0,target.E1),undefined,"A26 target authority absent at freeze");
  const frontier=materializeExactSequence(memory,[p1,p2]);
  const out=runFrontier(memory,rule,frontier);

  let truth=memory.ensure(out.M,target.E0);
  for(const expected of [target.E1,target.E2,target.E3,target.E4]){
    const next=metaStep(memory,truth);assert(next!==undefined,"A26 target meta-step");
    same(memory.poles(next).end,expected,"A26 converged authority execution");truth=next;
  }
  same(metaStep(memory,truth),undefined,"A26 terminal ZERO");

  const badTarget=materializeExactSequence(memory,[target.metaParent,target.E0]);
  const badPackage=materializeExactSequence(memory,[
    t1.candidate,memory.ensure(seedSequence(memory,source1),badTarget),
  ]);
  expectThrows(
    ()=>{runFrontier(memory,rule,materializeExactSequence(memory,[p1,badPackage]));},
    "A26 one malformed template package rejects full frontier",
  );
}

function staticGuards():void{
  const root=resolve(process.cwd(),"..");
  const own=readFileSync(join(root,"ts/test/research-v013-meta-authority-template-convergence-a26.test.ts"),"utf8");
  ...
  same(a.replace(/\s+/g,""),b.replace(/\s+/g,""),"A26 unchanged A23 metaStep");
  const runner=own.slice(own.indexOf("function runFrontier("),own.indexOf("\nfunction exercise(",own.indexOf("function runFrontier(")));
  for(const x of ["source1","source2","t1","t2","switch("])
    assert(!runner.includes(x),`A26 frontier runner excludes template identity branch ${x}`);
}

function main():void{
  exercise(false);exercise(true);staticGuards();
  console.log([
    "MTS v0.13 A26: META_AUTHORITY_TEMPLATE_CONVERGENCE=GREEN_SCOPED_RESEARCH",
    "TEMPLATE_FRONTIER_SIZE=2 SUCCESSFUL_TEMPLATE_INSTANTIATIONS=2",
    "DISTINCT_TARGET_CANDIDATES=1 DISTINCT_TARGET_PUBLICATIONS=1 DISTINCT_TARGET_META_CONTEXTS=1",
    "HOST_SELECTED_TEMPLATE_HANDLE=NO TARGET_AUTHORITY_ABSENT_AT_FREEZE=YES",
    "CORRESPONDENCE_PER_TEMPLATE=1 FROZEN_STRUCTURAL_ADMISSION=GREEN",
    "UNCHANGED_A23_META_STEP=YES MALFORMED_PACKAGE=REJECTS_FULL_FRONTIER",
    "EXTERNAL_TEMPLATE_FRONTIER_MEMBERSHIP=YES EXTERNAL_SEQUENCE_CORRESPONDENCE=YES",
    "SEMANTIC_INFORMATION_ELIMINATED=NOT_CLAIMED INDEPENDENT_MEMORIES=2",
