...
function assert(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(`v0.13 A29 Link-native target history: ${message}`);
}
...
function readChain(memory: Memory, envelope: LinkHandle): readonly LinkHandle[] {
  ...
  assert(e.start === envelope && e.end !== envelope, "A29 proper START-self envelope");
  const out: LinkHandle[] = [];
  const seen = new Set<LinkHandle>();
  let cursor = e.end;
  while (cursor !== memory.root) {
    assert(!seen.has(cursor), "A29 chain cycle");
    seen.add(cursor);
...
function referenceNextExecution(memory: Memory, executionRoot: LinkHandle): LinkHandle {
  ...
  for (const occurrence of occurrences) {
    ...
    assert(truth.start === K, "A29 reference truth keeps K");
...
function buildFamily(memory: Memory, b: RootBasis, seed: LinkHandle): Family {
  ...
  const at=(i:number):LinkHandle=>{const v=fresh[i];assert(v!==undefined,`A29 fresh ${i}`);return v;};
...
/** Exact A23 runtime consumer; A29 changes only producer-authority source selection. */
function metaStep(
  memory: Memory,
...
function producerTemplate(
  ...
  assert(executions.length>1,"A29 producer template has at least one transition");
...
function targetHistoryDescriptor(
  ...
  assert(executions.length>1,"A29 target history has more than one execution root");
...
function deriveTargetSeeds(
  ...
  while(true){
    assert(!seenOccurrences.has(cursor),"A29 target history occurrence cycle");
    seenOccurrences.add(cursor);
    const op=memory.poles(cursor),execution=op.end;
    assert(!seenExecutions.has(execution),"A29 target history repeats execution root");
    seenExecutions.add(execution);
    const ep=memory.poles(execution);
    if(context===undefined)context=ep.start;
    else same(ep.start,context,"A29 target execution roots share context");
    const fp=memory.poles(ep.end);
    assert(fp.start===ep.end&&fp.end!==ep.end,"A29 target execution has proper frontier envelope");
    reversed.push(execution);
    if(op.start===memory.root)break;
    cursor=op.start;
  }
  reversed.reverse();
  assert(reversed.length>1,"A29 target history nontrivial");
...
function deriveProducerSeeds(
  ...
  same(matches.length,1,"A29 exactly one structural producer seed path");
...
function derivedSeedMap(
  ...
  for(let i=0;i<source.length;i+=1){
    ...
    assert(prior===undefined||prior===target[i]!,"A29 derived seed correspondence conflict");
...
function instantiate(memory:Memory,source:LinkHandle,map:Map<LinkHandle,LinkHandle>):LinkHandle{
  ...
  const prior=map.get(source);assert(prior===undefined||prior===target,"A29 clone conflict");
...
function runInventory(
  ...
  assert(sources.length>0,"A29 non-empty broad template inventory");
  ...
  for(const source of sources){
    ...
    assert(admitted(memory,rule,candidate),"A29 structurally derived target candidate admitted");
    const values=readExactSequence(memory,candidate).values;
    const meta=values[M];assert(meta!==undefined,"A29 target M");
    const publication=memory.ensure(candidate,meta);
    candidates.add(candidate);publications.add(publication);metas.add(meta);
  }
  assert(compatibleCount>0,"A29 at least one structurally compatible template");
  same(candidates.size,1,"A29 derived compatible templates converge candidate");
  same(publications.size,1,"A29 derived compatible templates converge publication");
  same(metas.size,1,"A29 derived compatible templates converge M");
  return Object.freeze({
...
function exercise(noise:boolean):void{
  ...
  for(const expected of [source3.E1,source3.E2,source3.E3]){
    ...
    assert(next!==undefined,"A29 shorter source authority executes");
    same(memory.poles(next).end,expected,"A29 shorter authority transition");
    shortTruth=next;
  }
  ...
  same(memory.find(target.E0,target.E1),undefined,"A29 target authority absent at freeze");
...
function staticGuards():void{
  ...
  const own=readFileSync(
    join(root,"ts/test/research-v013-link-native-target-history-a29.test.ts"),"utf8",
  );
...
function main():void{
  ...
  console.log([
    "MTS v0.13 A29: LINK_NATIVE_TARGET_HISTORY=GREEN_SCOPED_RESEARCH",
    "TARGET_SEED_EXACT_SEQUENCE=REMOVED",
    "TARGET_DESCRIPTION=METAPARENT_TO_EXECUTION_HISTORY",
    "TARGET_EXECUTION_HISTORY=LINK_ANCESTRY",
    "SOURCE_SEEDS=DERIVED_FROM_PRODUCER_AUTHORITY_TOPOLOGY",
    "EXPLICIT_PER_TEMPLATE_SEED_CORRESPONDENCE=0",
    ...
    "UNCHANGED_A23_META_STEP=YES AMBIENT_TARGET_HISTORY=IGNORED",
    "MIXED_CONTEXT_TARGET_HISTORY=REJECTED LATE_AMBIENT_SOURCE_CANDIDATE=IGNORED",
    "EXTERNAL_SELECTED_TARGET_HISTORY=YES EXTERNAL_BROAD_INVENTORY_MEMBERSHIP=YES",
