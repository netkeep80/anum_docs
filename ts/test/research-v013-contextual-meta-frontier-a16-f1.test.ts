...
function assert(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(`v0.13 A16-F1 contextual meta frontier: ${message}`);
}
...
interface Artifact {
  readonly schema: "mts-v013-contextual-meta-frontier/research-v0.1";
...
function buildArtifact(noise: boolean): Artifact {
  ...
  const coord = (link: LinkHandle): number => {
    ...
    assert(n !== undefined, "A16-F1 coordinate");
    return n;
  };

  return Object.freeze({
    schema: "mts-v013-contextual-meta-frontier/research-v0.1" as const,
...
function replay(artifact: Artifact): Replay {
  same(artifact.schema, "mts-v013-contextual-meta-frontier/research-v0.1", "A16-F1 schema");
  const memory = restoreTopology(artifact.topology);
  const all = memory.allLinks();
  const at = (n: number): LinkHandle => {
    const x = all[n];
    assert(x !== undefined, "A16-F1 replay coordinate");
    return x;
  };
  return Object.freeze({
    memory,
    applications: Object.freeze(artifact.applicationCoordinates.map(at)),
...
function propagateCallTruth(
  ...
  for (const parent of callFrontier.occurrences) {
    ...
    assert(contextual.start === context, "A16-F1 contextual call starts at K");
...
/**
 * Explicit residual for this gate: an external event actualizes a selected
 * State->Event application and links current context K to that application.
 * A16-F1 does NOT claim this preparation authority has been source-removed.
 */
function prepareCallFrontier(
  r: Replay,
  ...
  for (const parent of parents) {
    ...
    same(truth.start, r.named.K!, "A16-F1 state truth keeps K");
    const state = truth.end;
    const matches = r.applications.filter((application) => {
      const p = r.memory.poles(application);
      return p.start === state && p.end === event;
    });
    same(matches.length, 1, "A16-F1 one selected State->Event application");
    calls.push(Object.freeze({
      path: parent.path,
      link: r.memory.ensure(r.named.K!, matches[0]!),
    }));
  }
...
function stateEnds(
  ...
  for (const truthLink of truthFrontier.links) {
    ...
    same(p.start, r.named.K!, "A16-F1 derived truth starts at K");
    states.add(p.end);
  }
  return states;
}
...
function normalizedProvenance(r: Replay, bundle: BundleValue): readonly string[] {
  ...
  return Object.freeze(bundle.occurrences.map((o) => {
    ...
    assert(n !== undefined, "A16-F1 consequence coordinate");
...
function assertBranchLocalStates(r: Replay): void {
  ...
  "A16-F1 first split keeps branch-local meta-state");
  const ka = readContext(r.memory, A.context);
  const kb = readContext(r.memory, B.context);
  same(ka.parent, kb.parent, "A16-F1 first split preserves lexical parent");
  ...
  assert(C.context !== D.context && C.tos !== D.tos && C.currentFn !== D.currentFn,
    "A16-F1 recursive split remains branch-local");
}
...
function exercise(artifact: Artifact): void {
  ...
  for (const name of ["K", "E0", "EA", "EB", "EC", "ED", "EZ", "event1", "event2", "event3"]) {
    assert(n[name] !== undefined, `A16-F1 named ${name}`);
  }
  assertBranchLocalStates(r);

  const start = initialTruth(r, n.E0!);
  setSame(stateEnds(r, start), [n.E0!], "A16-F1 initial K->E0 truth");

  const firstCalls = prepareCallFrontier(r, start, n.event1!, "forward");
  const first = propagateCallTruth(r.memory, n.K!, firstCalls, r.transitions);
  setSame(stateEnds(r, first), [n.EA!, n.EB!], "A16-F1 first split");
  same(first.occurrences.length, 2, "A16-F1 first branch count");

  const secondCallsForward = prepareCallFrontier(r, first, n.event2!, "forward");
  const secondCallsReverse = prepareCallFrontier(r, first, n.event2!, "reverse");
  ...
  setSame(stateEnds(r, secondForward), [n.EC!, n.ED!], "A16-F1 recursive split");
  same(secondForward.occurrences.length, 2,
    "A16-F1 EA terminates while EB splits");
  assert(valuesEqual(secondForward, secondReverse),
    "A16-F1 schedule order preserves extensional truth frontier");
  exactJson(
    normalizedProvenance(r, secondForward),
    normalizedProvenance(r, secondReverse),
    "A16-F1 schedule order preserves provenance",
  );
  exactJson(
    secondForward.occurrences.map((o) => o.path).sort(),
    [[1, 0], [1, 1]],
    "A16-F1 terminated EA branch contributes no truth children",
  );

  const thirdCallsForward = prepareCallFrontier(r, secondForward, n.event3!, "forward");
  const thirdCallsReverse = prepareCallFrontier(r, secondReverse, n.event3!, "reverse");
  ...
  setSame(stateEnds(r, thirdForward), [n.EZ!], "A16-F1 converged contextual truth");
  same(thirdForward.links.size, 1, "A16-F1 one extensional K->EZ truth Link");
  same(thirdForward.occurrences.length, 2,
    "A16-F1 convergence preserves two proof occurrences");
  assert(valuesEqual(thirdForward, thirdReverse),
    "A16-F1 convergence ignores scheduler order");
  exactJson(
    normalizedProvenance(r, thirdForward),
    normalizedProvenance(r, thirdReverse),
    "A16-F1 converged provenance ignores scheduler order",
  );

  // A physical transition added after freeze must remain semantically inert.
  const app = r.applications.find((application) => {
    const p = r.memory.poles(application);
    return p.start === n.EB! && p.end === n.event2!;
  });
  assert(app !== undefined, "A16-F1 selected EB/event2 application exists");
  ...
  setSame(stateEnds(r, afterAmbient), [n.EC!, n.ED!],
    "A16-F1 late ambient transition ignored");
  ...
  setSame(stateEnds(r, afterAmbientTruth), [n.EC!, n.ED!],
    "A16-F1 ambient K->state outside frontier ignored");
}
...
function staticKernelGuard(): void {
  ...
  for (const forbidden of [
    ...
    assert(!kernel.includes(forbidden),
      `A16-F1 propagation kernel excludes host semantic primitive ${forbidden}`);
  }
}
...
function main(): void {
  ...
  exactJson(a, b, "A16-F1 portable authority ignores unrelated allocation noise");
  ...
  console.log([
    ...
    "CALL_PREPARATION_HOST_RESIDUAL=1",
