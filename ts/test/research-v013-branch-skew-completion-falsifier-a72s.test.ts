...
/**
 * A71p strict Rule matcher, copied source-equivalently in semantics.
 *
 * Projection unification remains a different relation. Here every non-role
 * template node must preserve its two self-incidence bits exactly.
 */
...
function defineRule(
  ...
  // Function-local trigger. The exact structural Rule still decides whether
  // this particular application Context matches.
...
function defineRecursiveAllRules(
  ...
  // ALL(TRUE, nextHead, ...) -> ALL(nextHead, ...)
  {
...
/**
 * Existing A72-style reaction semantics:
 *
 * build the next current Scope only from Rule-produced successor Contexts.
 *
 * A Context with zero matching Rules contributes nothing to the successor
 * Scope. A72s intentionally tests whether that behavior is valid when one
 * branch has completed while a sibling branch is still reducing.
 */
...
/**
 * Open a nested call in the FIRST variadic argument only.
 *
 * Outer:
 *
 *   START(K -> (F -> PAIR(innerCall,tail)))
 *
 * Child:
 *
 *   START(outerContext -> innerCall)
 *
 * The helper is deliberately function-agnostic.
 */
...
/**
 * Resume one suspended variadic outer Context for every inner result branch.
 *
 * The child result replaces the first argument; the exact original tail is
...
function exercise(): void {
  ...
  // Round 1:
  //
  //   ALL(F,T,T) -> F          (completed branch)
  //   ALL(T,T,T) -> ALL(T,T)   (still active)
  //
  const round1 =
  ...
  // Round 2 exposes the lifecycle bug:
  //
  // the no-match FALSE Context contributes nothing to the next Scope, while
  // the TRUE branch continues. The already completed result is therefore lost
  // from current working state.
  const round2 =
  ...
  // Continue the surviving TRUE branch to completion.
  const round3 =
  ...
  // Publish only what remains current. This demonstrates the wrong relational
  // image produced by the naive lifecycle under branch-depth skew.
  ...
  // Correct program semantics should be both values:
  //
  // CHOICE -> {F,T}
  // ALL(F,T,T) -> F
  // ALL(T,T,T) -> T
  //
  // Canonical result bundle should therefore contain K->F and K->T.
