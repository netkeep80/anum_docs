...
function assert(c: unknown, m: string): asserts c {
  if (!c) throw new Error("v0.13 A72b binary AND dynamics: " + m);
}
...
function reactSingleActiveContext(
  ...
  assert(before.length === 1, "A72b requires exactly one active Context");
  const activeContext = before[0]!;
  readContext(memory, activeContext);

  const matches = discoverAllApplicableRules(memory, theory, activeContext);
  assert(matches.length > 0, "A72b requires at least one applicable Rule");
...
function buildFixture(): Fixture {
  ...
  const AND = memory.ensure(at(6), at(7));
  const parent = memory.ensure(at(8), at(9));

  assert(FALSE !== TRUE, "truth values are distinct");
  assert(AND !== FALSE && AND !== TRUE, "AND identity is distinct");

  defineBinaryTruthRule(memory, theory, b, at(10), AND, FALSE, FALSE, FALSE);
  defineBinaryTruthRule(memory, theory, b, at(11), AND, FALSE, TRUE, FALSE);
  defineBinaryTruthRule(memory, theory, b, at(12), AND, TRUE, FALSE, FALSE);
  defineBinaryTruthRule(memory, theory, b, at(13), AND, TRUE, TRUE, TRUE);

  return Object.freeze({ memory, theory, AND, FALSE, TRUE, parent });
}
...
function runAndCase(
  ...
  const { memory, theory, AND, parent } = f;
  const args = memory.ensure(left, right);
  const application = memory.ensure(AND, args);
...
function exercise(): void {
  ...
  runAndCase(f, f.FALSE, f.FALSE, f.FALSE, "AND(FALSE,FALSE)");
  runAndCase(f, f.FALSE, f.TRUE,  f.FALSE, "AND(FALSE,TRUE)");
  runAndCase(f, f.TRUE,  f.FALSE, f.FALSE, "AND(TRUE,FALSE)");
  runAndCase(f, f.TRUE,  f.TRUE,  f.TRUE,  "AND(TRUE,TRUE)");

  // The two argument positions are distinct physical occurrences even for a
  // commutative function. Later non-commutative functions can reuse exactly
  // this ordered carrier without changing application representation.
  ...
  const appFT = f.memory.ensure(f.AND, ft);
  const appTF = f.memory.ensure(f.AND, tf);
...
function main(): void {
  ...
  console.log([
    "MTS v0.13 A72b: BINARY_LOGIC_AND_WORKING_DYNAMICS=GREEN_SCOPED_RESEARCH",
    "PROGRAM=AND",
    "APPLICATION_TOPOLOGY=FUNCTION_TO_ORDERED_ARGUMENT_PAIR",
    "TRUTH_TABLE_ROWS=4",
    "AND_00=0 AND_01=0 AND_10=0 AND_11=1",
    ...
    "CUMULATIVE_REGRESSION=A72A_NOT_PLUS_A72B_AND",
