...
function assert(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(`v0.13 A16-F4 link-carried pair request: ${message}`);
}
...
interface Artifact {
  readonly schema: "mts-v013-link-carried-pair-request/research-v0.1";
...
function buildArtifact(noise: boolean): Artifact {
  ...
  const coord = (link: LinkHandle): number => {
    ...
    assert(n !== undefined, "A16-F4 coordinate");
    return n;
  };

  return Object.freeze({
    schema: "mts-v013-link-carried-pair-request/research-v0.1" as const,
...
function replay(artifact: Artifact): Replay {
  same(artifact.schema, "mts-v013-link-carried-pair-request/research-v0.1", "A16-F4 schema");
  const memory = restoreTopology(artifact.topology);
  const all = memory.allLinks();
  const at = (n: number): LinkHandle => {
    const x = all[n];
    assert(x !== undefined, "A16-F4 replay coordinate");
    return x;
  };
  return Object.freeze({
    memory,
...
function propagateCallTruth(
  ...
  for (const parent of callFrontier.occurrences) {
    ...
    assert(contextual.start === context, "A16-F4 contextual call starts at K");
...
/**
 * Residual request producer for this gate.
 *
 * Operand selection is encoded into Link data:
 *
 *   leftWitness  = K->X
 *   rightWitness = K->Y
 *   request      = leftWitness->rightWitness
 *   requestTruth = K->request
 *
 * The producer still receives the selected operand frontiers. A16-F4 tests
 * removal of those host operands from the semantic executor, not source
 * removal of request production itself.
 */
function encodePairRequests(
  memory: Memory,
  context: LinkHandle,
  leftTruthFrontier: BundleValue,
  rightTruthFrontier: BundleValue,
  schedule: "forward" | "reverse",
): BundleValue {
  const left = [...leftTruthFrontier.occurrences];
  if (schedule === "reverse") left.reverse();
  const requests: ResolvedOccurrence[] = [];

  for (const l of left) {
    const lp = memory.poles(l.link);
    assert(lp.start === context, "A16-F4 left request operand starts at K");

    for (const r of rightTruthFrontier.occurrences) {
      const rp = memory.poles(r.link);
      assert(rp.start === context, "A16-F4 right request operand starts at K");

      const request = memory.ensure(l.link, r.link);
      requests.push(Object.freeze({
        path: l.path,
        link: memory.ensure(context, request),
      }));
    }
  }

  return resolveFlatBundle(memory, Object.freeze(requests));
}

/**
 * Semantic executor for Link-carried pair requests.
 *
 * The executor receives only K->Q. Q itself carries the exact two contextual
 * truth witnesses to combine. It does not receive separate operand frontiers
 * and does not know State/Event/application semantics.
 */
...
function contextualTruth(
  ...
  return resolveFlatBundle(r.memory, Object.freeze([
    Object.freeze({
      path: Object.freeze([]),
      link: r.memory.ensure(r.named.K!, value),
    }),
  ]));
}

function initialTruth(r: Replay, state: LinkHandle): BundleValue {
  return contextualTruth(r, state);
}
...
function stateEnds(
  ...
  for (const truthLink of truthFrontier.links) {
    ...
    same(p.start, r.named.K!, "A16-F4 derived truth starts at K");
    states.add(p.end);
  }
  return states;
}
...
function normalizedProvenance(r: Replay, bundle: BundleValue): readonly string[] {
  ...
  return Object.freeze(bundle.occurrences.map((o) => {
    ...
    assert(n !== undefined, "A16-F4 consequence coordinate");
...
function assertBranchLocalStates(r: Replay): void {
  ...
  "A16-F4 first split keeps branch-local meta-state");
  const ka = readContext(r.memory, A.context);
  const kb = readContext(r.memory, B.context);
  same(ka.parent, kb.parent, "A16-F4 first split preserves lexical parent");
  ...
  assert(C.context !== D.context && C.tos !== D.tos && C.currentFn !== D.currentFn,
    "A16-F4 recursive split remains branch-local");
}
...
function exercise(artifact: Artifact): void {
  ...
  for (const name of ["K", "E0", "EA", "EB", "EC", "ED", "EZ", "event1", "event2", "event3"]) {
    assert(n[name] !== undefined, `A16-F4 named ${name}`);
  }
  assertBranchLocalStates(r);

  const start = initialTruth(r, n.E0!);
  setSame(stateEnds(r, start), [n.E0!], "A16-F4 initial K->E0 truth");

  const event1Truth = contextualTruth(r, n.event1!);
  const firstRequests = encodePairRequests(
    r.memory, n.K!, start, event1Truth, "forward",
  );
  const firstCalls = executePairRequests(r.memory, n.K!, firstRequests);
  const first = propagateCallTruth(r.memory, n.K!, firstCalls, r.transitions);
  setSame(stateEnds(r, first), [n.EA!, n.EB!], "A16-F4 first split");
  same(first.occurrences.length, 2, "A16-F4 first branch count");

  const event2Truth = contextualTruth(r, n.event2!);
  const secondRequestsForward = encodePairRequests(
    r.memory, n.K!, first, event2Truth, "forward",
  );
  const secondRequestsReverse = encodePairRequests(
    r.memory, n.K!, first, event2Truth, "reverse",
  );
  const secondCallsForward = executePairRequests(
    r.memory, n.K!, secondRequestsForward,
  );
  const secondCallsReverse = executePairRequests(
    r.memory, n.K!, secondRequestsReverse,
  );
  ...
  setSame(stateEnds(r, secondForward), [n.EC!, n.ED!], "A16-F4 recursive split");
  same(secondForward.occurrences.length, 2,
    "A16-F4 EA terminates while EB splits");
  assert(valuesEqual(secondForward, secondReverse),
    "A16-F4 schedule order preserves extensional truth frontier");
  exactJson(
    normalizedProvenance(r, secondForward),
    normalizedProvenance(r, secondReverse),
    "A16-F4 schedule order preserves provenance",
  );
  exactJson(
    secondForward.occurrences.map((o) => o.path).sort(),
    [[1, 0], [1, 1]],
    "A16-F4 terminated EA branch contributes no truth children",
  );

  const event3Truth = contextualTruth(r, n.event3!);
  const thirdCallsForward = executePairRequests(
    r.memory,
    n.K!,
    encodePairRequests(r.memory, n.K!, secondForward, event3Truth, "forward"),
  );
  const thirdCallsReverse = executePairRequests(
    r.memory,
    n.K!,
    encodePairRequests(r.memory, n.K!, secondReverse, event3Truth, "reverse"),
  );
  ...
  setSame(stateEnds(r, thirdForward), [n.EZ!], "A16-F4 converged contextual truth");
  same(thirdForward.links.size, 1, "A16-F4 one extensional K->EZ truth Link");
  same(thirdForward.occurrences.length, 2,
    "A16-F4 convergence preserves two proof occurrences");
  assert(valuesEqual(thirdForward, thirdReverse),
    "A16-F4 convergence ignores scheduler order");
  exactJson(
  ...
  // resulting call truth must evaluate to ZERO.
  {
    ...
    const unknownCalls = executePairRequests(
      r.memory,
      n.K!,
      encodePairRequests(r.memory, n.K!, onlyEA, event3Truth, "forward"),
    );
    const unknown = propagateCallTruth(r.memory, n.K!, unknownCalls, r.transitions);
    same(unknown.links.size, 0,
      "A16-F4 generic application without selected continuation yields ZERO");
  ...
  const firstAgain = propagateCallTruth(
    ...
    executePairRequests(
      r.memory,
      n.K!,
      encodePairRequests(r.memory, n.K!, start, event1Truth, "forward"),
    ),
    r.transitions,
  );
  ...
  const mixedEventTruth = resolveFlatBundle(r.memory, Object.freeze([
    ...
    Object.freeze({
      ...
      link: r.memory.ensure(n.K!, unrelatedEvent),
    }),
  ]));
  const mixedRequests = encodePairRequests(
    r.memory, n.K!, start, mixedEventTruth, "forward",
  );
  const mixedCalls = executePairRequests(r.memory, n.K!, mixedRequests);
  const mixedResult = propagateCallTruth(
    r.memory, n.K!, mixedCalls, r.transitions,
  );
  ...
  const ambientRequest = r.memory.ensure(startTruthLink, unrelatedTruthLink);
  r.memory.ensure(n.K!, ambientRequest);

  const firstSelectedAgain = propagateCallTruth(
    r.memory,
    n.K!,
    executePairRequests(r.memory, n.K!, firstRequests),
    r.transitions,
  );
  ...
  const malformedRequestTruth = r.memory.ensure(n.K!, malformedRequest);
  const malformedFrontier = resolveFlatBundle(r.memory, Object.freeze([
    Object.freeze({
      path: Object.freeze([]),
      link: malformedRequestTruth,
    }),
  ]));
  expectThrows(
    () => { executePairRequests(r.memory, n.K!, malformedFrontier); },
  ...
  setSame(stateEnds(r, afterAmbientTruth), [n.EC!, n.ED!],
    "A16-F4 ambient K->state outside frontier ignored");
}

function staticKernelGuard(): void {
  const repoRoot = resolve(process.cwd(), "..");
  const source = readFileSync(
    join(repoRoot, "ts/test/research-v013-link-carried-pair-request-a16-f4.test.ts"),
    "utf8",
  );
  const transitionStart = source.indexOf("function propagateCallTruth(");
  const transitionEnd = source.indexOf("/**\n * Residual request producer", transitionStart);
  const executorStart = source.indexOf("function executePairRequests(");
  const executorEnd = source.indexOf("function contextualTruth(", executorStart);
  assert(
    transitionStart >= 0 &&
    transitionEnd > transitionStart &&
    executorStart > transitionEnd &&
    executorEnd > executorStart,
    "A16-F4 kernel source slices",
  );

  const transitionKernel = source.slice(transitionStart, transitionEnd);
  const executorKernel = source.slice(executorStart, executorEnd);
  ...
  for (const forbidden of [
    ...
    assert(!transitionKernel.includes(forbidden),
      `A16-F4 propagation kernel excludes host semantic primitive ${forbidden}`);
  }
  ...
  for (const forbidden of [
    ...
    assert(!executorKernel.includes(forbidden),
      `A16-F4 request executor excludes host operand primitive ${forbidden}`);
  }

  assert(executorKernel.includes("requestFrontier: BundleValue"),
    "A16-F4 executor receives only Link-carried request frontier");
  assert(executorKernel.includes("memory.poles(requestTruth.end)"),
    "A16-F4 executor reads operand selection from request Link");
}

function main(): void {
  const a = buildArtifact(false);
  const b = buildArtifact(true);
  exactJson(a, b, "A16-F4 portable authority ignores unrelated allocation noise");
  ...
  console.log([
    ...
    "RAW_EVENT_CONTROL_PARAMETER=REMOVED",
    "EVENT_REPRESENTATION=K_TO_EVENT_TRUTH_LINK",
    "PAIR_ACTUALIZATION=LINK_CARRIED_REQUEST",
    "REQUEST_SHAPE=K_TO_LEFTTRUTH_TO_RIGHTTRUTH",
    "EXECUTOR_OPERAND_FRONTIER_PARAMETERS=0",
    "REQUEST_PRODUCER_HOST_RESIDUAL=1",
    "LATE_AMBIENT_PAIR_REQUEST=IGNORED",
    "CROSS_CONTEXT_REQUEST=REJECTED",
    "ABSENT_PAIR_REQUEST=ZERO",
    "AMBIENT_EVENT_TRUTH_OUTSIDE_OPERAND_FRONTIER=IGNORED",
    "MIXED_EVENT_TRUTH_UNKNOWN_BRANCH=ZERO",
