...
function target(
  ...
  const targetOccurrence = memory.ensure(derivationRule, materializeExactSequence(memory, slots));
  return Object.freeze({ derivationRule, identity, assumptions: Object.freeze(assumptions), targetOccurrence });
}
...
function carrier(
  ...
  return materializeExactSequence(memory, [
    ...
    sourceDictionary,
    targetDictionary,
    entries(roleBindings),
    entries(groundBindings),
  ]);
}
...
function evidence(
  ...
  return Object.freeze({
    source,
    specialization,
    targetIdentity: fixture.identity,
    targetAssumptions: fixture.assumptions,
    targetOccurrence: fixture.targetOccurrence,
  });
}
...
function main(): void {
  ...
  // Pure Role->Role, including nested/repeated occurrences.
  const x = fresh();
  const n = fresh();
  ...
  const roleReplay = replayStructuralDerivedDerivationSpecialization(
    memory,
    evidence(sourceRole.evidence, roleCarrier, targetRole),
  );
  same(roleReplay.theory, theory, "pure role specialization Theory");
  same(roleReplay.targetConclusionTemplate, tri(n, n, n), "nested repeated role target");
  same(memory.linkCount, roleBefore, "pure role specialization is read-only");

  // Pure grounding into an empty target scope.
  const ground = fresh();
  ...
  const groundCarrier = carrier(memory, theory, dx, dEmpty, [], [[x, ground]]);
  const groundReplay = replayStructuralDerivedDerivationSpecialization(
    memory,
    evidence(sourceGround.evidence, groundCarrier, targetGround),
  );
  same(groundReplay.targetConclusionTemplate, unary(ground), "pure grounding target");

  // Mixed non-injective specialization with exact premise-slot aliasing.
  ...
  const sourceCurrent = tri(a, b, c);
  const sourceStepB = transition(b, b1);
  const sourceStepC = transition(c, c1);
  const sourceNext = tri(a, b1, c1);
  const source = admittedGeneric(
    memory,
    theory,
    dSource,
    [sourceCurrent, sourceStepB, sourceStepC],
    sourceNext,
  );
  ...
  const mixedTarget = target(
    ...
    theory,
    dTarget,
    [mappedCurrent, mappedStep, mappedStep],
    mappedNext,
  );
  ...
  same(mixedReplay.targetAssumptionCount, 2, "aliased slots use two unique semantic assumptions");
  same(mixedReplay.premiseSlotCount, 3, "structural slot multiplicity is preserved");
  same(mixedReplay.targetConclusionTemplate, mappedNext, "mixed conclusion");
  same(memory.linkCount, mixedBefore, "mixed specialization is read-only");
  assert(memory.find(theory, mixedTarget.derivationRule) === undefined, "target DR remains unadmitted");

  // Host decorations grant no authority and do not change valid replay.
  const decorated = { ...mixedEvidence, cached: true, weakening: true, name: "trusted", callback: () => true };
  same(
    replayStructuralDerivedDerivationSpecialization(memory, decorated).targetConclusionTemplate,
    mappedNext,
    "host decorations are inert",
  );

  const wrongDictionary = defineStructuralRoleDictionary(memory, [fresh()]);
  const foreignTheory = fresh();
  const extra = fresh();
  const malformed = materializeExactSequence(memory, [theory, dSource, dTarget, extra]);
  expectError("invalid-specialization-carrier", () =>
    replayStructuralDerivedDerivationSpecialization(memory, evidence(source.evidence, malformed, mixedTarget)));
  expectError("theory-mismatch", () =>
    replayStructuralDerivedDerivationSpecialization(memory, evidence(
      source.evidence,
      carrier(memory, foreignTheory, dSource, dTarget, [[b, n], [c, n], [b1, n1], [c1, n1]], [[a, ground]]),
      mixedTarget,
    )));
  expectError("source-dictionary-mismatch", () =>
    replayStructuralDerivedDerivationSpecialization(memory, evidence(
      source.evidence,
      carrier(memory, theory, wrongDictionary, dTarget, [], []),
      mixedTarget,
    )));
  expectError("target-dictionary-mismatch", () =>
    replayStructuralDerivedDerivationSpecialization(memory, evidence(
      source.evidence,
      carrier(memory, theory, dSource, wrongDictionary, [[b, n], [c, n], [b1, n1], [c1, n1]], [[a, ground]]),
      mixedTarget,
    )));
  expectError("missing-source-role", () =>
    replayStructuralDerivedDerivationSpecialization(memory, evidence(
      source.evidence,
      carrier(memory, theory, dSource, dTarget, [[b, n], [c, n], [b1, n1]], [[a, ground]]),
      mixedTarget,
    )));
  expectError("duplicate-source-role-binding", () =>
    replayStructuralDerivedDerivationSpecialization(memory, evidence(
      source.evidence,
      carrier(memory, theory, dSource, dTarget, [[b, n], [b, n1], [c, n], [b1, n1], [c1, n1]], [[a, ground]]),
      mixedTarget,
    )));
  expectError("binding-partition-overlap", () =>
    replayStructuralDerivedDerivationSpecialization(memory, evidence(
      source.evidence,
      carrier(memory, theory, dSource, dTarget, [[a, n], [b, n], [c, n], [b1, n1], [c1, n1]], [[a, ground]]),
      mixedTarget,
    )));
  expectError("undeclared-source-role", () =>
    replayStructuralDerivedDerivationSpecialization(memory, evidence(
      source.evidence,
      carrier(memory, theory, dSource, dTarget, [[extra, n], [b, n], [c, n], [b1, n1], [c1, n1]], [[a, ground]]),
      mixedTarget,
    )));
  expectError("target-role-not-member", () =>
    replayStructuralDerivedDerivationSpecialization(memory, evidence(
      source.evidence,
      carrier(memory, theory, dSource, dTarget, [[b, n], [c, n], [b1, n1], [c1, extra]], [[a, ground]]),
      mixedTarget,
    )));
  expectError("grounded-target-role-capture", () =>
    replayStructuralDerivedDerivationSpecialization(memory, evidence(
      source.evidence,
      carrier(memory, theory, dSource, dTarget, [[b, n], [c, n], [b1, n1], [c1, n1]], [[a, n]]),
      mixedTarget,
    )));

  const wrongConclusion = target(
    memory, theory, dTarget, [mappedCurrent, mappedStep, mappedStep], tri(ground, n1, n),
  );
  expectError("conclusion-mismatch", () =>
    replayStructuralDerivedDerivationSpecialization(memory, evidence(source.evidence, mixedCarrier, wrongConclusion)));
  ...
  expectError("assumption-image-mismatch", () =>
    replayStructuralDerivedDerivationSpecialization(memory, {
      ...mixedEvidence,
      targetAssumptions: Object.freeze([...mixedTarget.assumptions, Object.freeze({ occurrence: extra, template: extra })]),
    }));
  expectError("assumption-image-mismatch", () =>
    replayStructuralDerivedDerivationSpecialization(memory, {
      ...mixedEvidence,
      targetAssumptions: Object.freeze(mixedTarget.assumptions.slice(0, 1)),
    }));

  const slotPoles = memory.poles(mixedTarget.targetOccurrence);
  const slotSequence = materializeExactSequence(memory, [
  ...
  expectError("premise-slot-mismatch", () =>
    replayStructuralDerivedDerivationSpecialization(memory, {
      ...mixedEvidence,
      targetOccurrence: memory.ensure(slotPoles.start, slotSequence),
    }));
  const shortSequence = materializeExactSequence(memory, [
    mixedTarget.assumptions[0]!.occurrence,
    mixedTarget.assumptions[1]!.occurrence,
  ]);
  expectError("premise-slot-mismatch", () =>
    replayStructuralDerivedDerivationSpecialization(memory, {
      ...mixedEvidence,
      targetOccurrence: memory.ensure(slotPoles.start, shortSequence),
    }));

  // One occurrence cannot alias two non-identical exact semantic claims.
  ...
  // A write observed through a ReadMemory facade must dominate nested errors.
  const isolated = new Memory();
  const basis = ensureRootBasis(isolated);
  const writeStart = isolated.ensure(basis.U, basis.L);
  const writeEnd = isolated.ensure(basis.C, basis.O);
