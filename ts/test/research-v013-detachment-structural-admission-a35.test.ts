...
function assert(condition: unknown, message: string): asserts condition {
  if (!condition) throw new Error(`v0.13 A35 detachment admission: ${message}`);
}
...
function staticGuards(): void {
  ...
  const own = readFileSync(
    join(root, "ts/test/research-v013-detachment-structural-admission-a35.test.ts"),
    "utf8",
  );
  ...
  for (const forbidden of [
    ...
    assert(!compiler.includes(forbidden), `A35 compiler excludes semantic truth test ${forbidden}`);
  }
  ...
  for (const forbidden of [
    ...
    assert(!kernel.includes(forbidden), `A35 detachment kernel excludes domain primitive ${forbidden}`);
  }
  ...
  assert(oldAdmission.includes("memory.find(s,e)!==t"),
    "A35 explicitly targets A34 host pair-identity truth test");
  ...
  assert(
    ...
    "A35 keeps A16 detachment evidence in scope",
  );
}
...
function main(): void {
  ...
  console.log([
    "MTS v0.13 A35: DETACHMENT_STRUCTURAL_ADMISSION=GREEN_SCOPED_RESEARCH",
