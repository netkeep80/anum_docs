...
function assert(condition:unknown,message:string):asserts condition{
  if(!condition) throw new Error(`v0.13 F5-F6 seed correspondence compression: ${message}`);
}
...
interface FrozenArtifact{
  readonly schema:"mts-v013-seed-correspondence-compression-f5-f6/v0.1";
  ...
  readonly correspondenceSourcesCoordinate:number;
}
...
function buildFrozen():FrozenArtifact{
  ...
  const qPrefixSequence=materializeExactSequence(memory,qPrefixes);
  const rPrefixSequence=materializeExactSequence(memory,rPrefixes);
  const requestCorrespondence=memory.ensure(qRequest,rRequest);
  const prefixCorrespondence=memory.ensure(qPrefixSequence,rPrefixSequence);
  const correspondenceSources=materializeExactSequence(
    memory,[requestCorrespondence,prefixCorrespondence],
  );
  ...
  return Object.freeze({
    schema:"mts-v013-seed-correspondence-compression-f5-f6/v0.1" as const,
    ...
    correspondenceSourcesCoordinate:coord(
      canonical.coordinates,correspondenceSources,"correspondence sources coordinate",
    ),
  });
}
...
interface InstantiateResult{
  readonly publication:LinkHandle;
  readonly candidate:LinkHandle;
  readonly correspondenceSourceCount:number;
  readonly zippedPositionCount:number;
...
function instantiateTemplate(
  ...
  correspondenceSources:LinkHandle,
  fullAuthority:boolean,
):InstantiateResult{
  const mapping=new Map<LinkHandle,LinkHandle>();
  const sources=readExactSequence(memory,correspondenceSources).values;
  let zippedPositionCount=0;
  for(const source of sources){
    const pair=memory.poles(source);
    const from=readExactSequence(memory,pair.start).values;
    const to=readExactSequence(memory,pair.end).values;
    same(from.length,to.length,"correspondence source cardinality");
    zippedPositionCount+=from.length;
    from.forEach((value,index)=>addMapping(mapping,value,to[index]!));
  }
  ...
  return Object.freeze({
    ...
    correspondenceSourceCount:sources.length,
    zippedPositionCount,
    derivedSeedCount,
...
function selectCorrespondenceSources(
  ...
  const sources=readExactSequence(memory,original).values;
  ...
  const shortened=materializeExactSequence(memory,targetPrefixes.slice(0,-1));
  const malformed=memory.ensure(prefixPair.start,shortened);
  return materializeExactSequence(memory,[requestSource,malformed]);
}
function runTemplate(
  ...
  const memory=restoreTopology(artifact.topology),all=memory.allLinks();
  const correspondenceSources=selectCorrespondenceSources(
    memory,at(all,artifact.correspondenceSourcesCoordinate),mode,
  );
...
function main():void{
  ...
  exactJson(a.topology,b.topology,"independent correspondence-derived instantiations agree");
  exactJson(a.topology,reference.topology,"compressed correspondence topology equals domain reference");
  same(a.result.correspondenceSourceCount,2,"two sequence correspondence sources");
  assert(a.result.derivedSeedCount>0,"generic zipper derives seed mappings");
  assert(a.result.ordinary>0&&a.result.startSelf>0,"generic cloning exercised");

  console.log([
    "MTS v0.13 F5-F6:",
    "SEED_CORRESPONDENCE_COMPRESSION=GREEN_SCOPED_RESEARCH",
    "EXPLICIT_PER_LINK_SEED_PAIRS=0",
    `CORRESPONDENCE_SEQUENCE_SOURCES=${a.result.correspondenceSourceCount}`,
    `ZIPPED_POSITIONS=${a.result.zippedPositionCount}`,
    ...
    "EXTERNAL_CORRESPONDENCE_SOURCE_SELECTION=2",
    "EXTERNAL_PREFIX_SEQUENCE_PREPARATION=YES",
